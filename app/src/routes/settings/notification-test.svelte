<script lang="ts">
  ...
  async function testForceActivate() {
    forcedActivateStatus = "App activation test in progress..."
    try {
      await invoke("test_force_activate")
      forcedActivateStatus = "App activation command executed"
    } catch (error) {
      forcedActivateStatus = `Error occurred: ${error}`
...
<div class="p-4">
  ...
  <div class="mb-6 bg-base-200 p-4 rounded-lg">
    <p class="text-sm text-base-content/80 mb-2">
      알림을 테스트하려면 아래 필드를 작성하고 '알림 표시' 버튼을 클릭하세요. 알림이 표시되면 알림을 클릭하여 앱이 활성화되고 키워드 검색이 실행되는지 확인하세요.
    </p>
    ...
    <div class="flex flex-col gap-2">
      ...
      <button class="btn btn-primary mt-2" on:click={showTestNotification}>알림 표시</button>
  ...
  <div class="mb-6 bg-base-200 p-4 rounded-lg">
    ...
    <p class="text-sm text-base-content/80 mb-2">앱을 최소화한 후 이 버튼을 다른 응용 프로그램에서 클릭하면 앱이 활성화되는지 테스트할 수 있습니다.</p>

    <div class="flex gap-2">
      <button class="btn btn-secondary" on:click={testForceActivate}>앱 강제 활성화</button>

      <button class="btn btn-accent" on:click={testSearchKeyword}>키워드 검색 테스트</button>
    </div>
  ...
  <div class="bg-base-200 p-4 rounded-lg">
    ...
    <ul class="list-disc ml-5 text-sm">
      <li>
        <code>%TEMP%\mcplink_debug.log</code>
        : 앱 디버그 로그
      </li>
      <li>
        <code>%TEMP%\mcplink_notification.log</code>
        : 알림 시스템 로그
      </li>
      <li>
        <code>%TEMP%\mcplink_activation.log</code>
        : 앱 활성화 로그
      </li>
      <li>
        <code>%TEMP%\mcplink_notification_click.log</code>
        : 알림 클릭 로그
      </li>
      <li>
        <code>%TEMP%\mcplink_test.log</code>
        : 테스트 명령 로그
      </li>
    </ul>
  </div>
