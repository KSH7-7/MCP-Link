<script lang="ts">
  ...
  let unlistenSearchKeyword: UnlistenFn | undefined // Tauri v2.0 리스너 정리 함수
  let unlistenActivationComplete: UnlistenFn | undefined // Tauri v2.0 리스너 정리 함수
  let unlistenNavigate: UnlistenFn | undefined // 네비게이션 리스너
  // END: Added variables
  ...
  onMount(() => {
    ...
    return () => {
      ...
      if (unlistenActivationComplete) unlistenActivationComplete()
      ...
      console.log("[MCP-List] 모든 이벤트 리스너 및 리소스 정리 완료")
    }
  })

  // 비동기 이벤트 리스너 설정 함수
  async function setupListeners() {
    // 1. Tauri v2.0 호환 search-keyword 이벤트 리스닝
    try {
      console.log("[MCP-List] Tauri v2.0 search-keyword 이벤트 리스너 설정 중...")

      // 이벤트 리스너 등록
      unlistenSearchKeyword = await listen("search-keyword", (event) => {
        try {
          console.log("[MCP-List] search-keyword 이벤트 수신:", event)

          // 키워드 추출 (문자열이나 객체 형태에 대응)
          const keyword =
            typeof event.payload === "string"
              ? event.payload // 이미 문자열이면 그대로 사용
              : typeof event.payload === "object" && event.payload && (event.payload as any).keyword
                ? (event.payload as any).keyword // 객체에서 keyword 속성 추출
                : null

          if (keyword) {
            console.log("[MCP-List] 수신된 검색 키워드:", keyword)
            ...
            try {
              ...
              console.log("[MCP-List] 검색창에 키워드 설정됨:", keyword)
              ...
              console.error("[MCP-List] 검색창 키워드 설정 오류:", e)
            }
          } else {
            console.warn("[MCP-List] search-keyword 이벤트에서 키워드를 찾을 수 없음")
          }
        } catch (e) {
          console.error("[MCP-List] search-keyword 이벤트 처리 오류:", e)
        }
      })

      console.log("[MCP-List] search-keyword 이벤트 리스너 설정 완료")
    } catch (e) {
      console.error("[MCP-List] Error setting up search-keyword listener:", e)
    }

    // 2. activation-complete 이벤트 리스닝
    try {
      unlistenActivationComplete = await listen("activation-complete", (event) => {
        // 앱이 활성화되었을 때 추가 조치
        console.log("[MCP-List] activation-complete 이벤트 수신")
        ...
        if (urlKeyword) {
          console.log("[MCP-List] 이미 URL에 키워드가 있어 알림 키워드 처리 무시:", urlKeyword)
        ...
        const pendingKeyword = sessionStorage.getItem("pendingSearchKeyword")
        if (pendingKeyword) {
          console.log("[MCP-List] 앱 활성화: 세션 스토리지에서 키워드 발견:", pendingKeyword)
          searchTermFromQuery = pendingKeyword
          isRecommendedSearch = true
          searchAndDisplay(pendingKeyword)
        ...
        if (lastKeyword && (!searchTermFromQuery || searchTermFromQuery !== lastKeyword)) {
          console.log("[MCP-List] 앱 활성화: 로컬 스토리지에서 키워드 발견:", lastKeyword)
          searchTermFromQuery = lastKeyword
          isRecommendedSearch = true
          searchAndDisplay(lastKeyword)
      ...
      console.error("[MCP-List] Error setting up activation-complete listener:", e)
    }

    // 메인 윈도우 네비게이션 이벤트 리스너
    try {
      unlistenNavigate = await listen("navigate-to-mcp-list-with-keyword", async (event) => {
      ...
      console.error("[MCP-List] 네비게이션 이벤트 설정 오류:", e)
    }
  }
  ...
  <div class="mt-3 px-4">
    ...
    {:else}
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {#each mcpCards as card, index (`${card.id}-${index}`)}
          <MCPCard {...card} />
        {/each}
      </div>
